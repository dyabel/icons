...
if __name__ == '__main__':
    ...
    x_batch = batches[0][0]
    # print(len(x_batch))
    x_batch = torch.cat((x_batch.cuda(), torch.randn(len(x_batch),1).cuda()), dim=-1)
    num = len(x_batch)
    ...
    for epoch in range(100):
        ...
        for i in range(num//batch_size):
            ...
            loss.backward()
            loss_batch += loss
            switch_loss_batch += ((bool_func(x[:,0], x[:,1])).float() - switch).mean()
            ori_error_batch += (((bool_func(x[:,0], x[:,1])) * sin_x + (1-bool_func(x[:,0], x[:,1])) * cos_x - labels)**2).mean()
            optimizer.step()
        print('epoch', epoch)

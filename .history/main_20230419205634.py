...
class SNN(nn.Module):  # Example net for MNIST
    def __init__(self):
        ...
        self.fc1_s = tdLayer(nn.Linear(2, 16))
        self.fc2_s =tdLayer(nn.Linear(16, 1))
...
class SNN_jelly(nn.Module):
    def __init__(self, tau):
        super().__init__()

        self.layer = nn.Sequential(
            layer.Flatten(),
            layer.Linear(2, 8, bias=False),
            neuron.LIFNode(tau=tau, surrogate_function=surrogate.ATan()),
            layer.Linear(8, 1, bias=False),
            neuron.LIFNode(tau=tau, surrogate_function=surrogate.ATan()),
            )
...
if __name__ == '__main__':
    ...
    sin_approximator = UnaryApproximator(torch.sin, 16, domain, 3).cuda()
    sin_approximator.load_state_dict(torch.load('funcs_new/sin_16.pkl'))
    cos_approximator = UnaryApproximator(torch.cos, 16, domain, 3).cuda()
    sin_approximator.load_state_dict(torch.load('funcs_new/cos_16.pkl'))
    # snn = ANN().cuda()

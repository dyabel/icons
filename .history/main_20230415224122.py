...
if __name__ == '__main__':
    ...
    num = 10000
    x_batch = torch.rand(num, 2).cuda() * 20 - 10
    for _ in range(10):
        loss_batch = 0
        ori_error_batch = 0.
        switch_loss_batch = 0.
        for i in range(num):
            ...
            loss.backward()
            loss_batch += loss
            switch_loss_batch += (x[0]<x[1]).float() - switch
            ori_error_batch +=((3*x[0]<x[1]) * sin_x + (3*x[0]>=x[1]) * cos_x - label)**2
            optimizer.step()
        print('average switch loss', switch_loss_batch/num)
        print('average loss: ', loss_batch/num)
        print('average original error:', ori_error_batch/num)
